...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def save_and_close(self):
        self.data = df_to_function(self.canvas.get_data())
        self.close()
...
def df_to_function(df, clamp = True):
    ...
    - clamp = True: fuera de rango usa el valor del borde (constante).
    - clamp = False: fuera de rango devolverá NaN.
    """
    # Asegura arrays 1D y orden por tiempo
    ...
    # Elimina duplicados exactos en 't' quedándote con el último valor
    ...
    if len(uniq_t) != len(t):
        ...
        for ti, yi in zip(t, y):
            last_vals[ti] = yi
        t = np.array(list(last_vals.keys()), dtype = float)
        y = np.array(list(last_vals.values()), dtype = float)
        idx = np.argsort(t)
        t = t[idx]
        y = y[idx]
    ...
    def f(tq):
        """
        tq puede ser escalar o array-like.
        Devuelve escalar si tq es escalar; np.ndarray si tq es vector.
        """
        tq_arr = np.asarray(tq, dtype = float)
        vals = np.interp(tq_arr, t, y, left = left, right = right)
        # Devuelve escalar si la entrada era escalar
